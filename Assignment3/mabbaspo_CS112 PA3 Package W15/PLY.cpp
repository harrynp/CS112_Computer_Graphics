			...
			for (int v=0; v<nv; v++) 
				{
					...
					getModelViewMatrix(modelViewMatrix, M);

					//L is unit vector towards the light source
					getVector(L, light_pos);
					...
					Vector4f lightAmbient, lightModelAmbient, lightDiffuse, lightSpecular;
					...
					//attenuation = 1 / (a + bd + c(d^2))
					float attenuation = 1 / (constantAttenuation + linearAttenuation * d + quadraticAttenuation * pow(d, 2));

					//Start of Phong equation with ambient
					multVectors(dif, lightModelAmbient, ambient);
					multVectors(amb, lightAmbient, ambient);
					scale(amb, attenuation);
					//total = lightModelAmbient * ambient + lightAmbient * ambient * attenuation
					add(total, dif, amb);

					//use lambert's cosine law to see if lambert > 0, if lambert > 0 then specular is 0
					float lambert = dotProd(normals[v], H);
					if (lambert > 0){
						//diffuse
						...
						Vector3f normalizedView;
						getVector(normalizedView, viewer_pos);
						normalize(normalizedView);

						//lambert reflection
						scale(dif, lambert * 2, normals[v]);
						//reflect(-L, N)
						sub(spe, dif, H);

						//specular
						float dotProduct = dotProd(spe, normalizedView);
						float specularFactor = dotProduct > 0 ? pow(dotProduct, shininess[0]) : 0; // pow(0.0, shininess[0])

						multVectors(spe, lightSpecular, specular);
						scale(spe, specularFactor * attenuation, spe);
						add(total, total, spe);
					}
