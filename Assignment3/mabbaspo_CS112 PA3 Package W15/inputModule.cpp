...
GLfloat light_current_pos[] = { -100.0, 100.0, 100.0, 0 };
int toggle= 0;
...
void readKeyboard(unsigned char key, int x, int y)
{
  ...
  case 'R': 
   // reset initial view parameters
	...
	lightAngle = 20;
	lightAngle2 = 30;
	light_current_pos[0] = -100.0;
	light_current_pos[1] = 100.0;
	light_current_pos[2] = 100.0;
	light_current_pos[3] = 0.0;
    break;
  case 'h':
  case 'H':
...
void mouseMoveHandler(int x, int y)
{
  ...
  case 2:
	  if (toggle){
		  light_current_pos[0] = light_current_pos[0] - (x - startX) / 100.0;
		  light_current_pos[1] = light_current_pos[1] - (y - startY) / 100.0;
	  }
	  else{
  ...
  case 3:
	  if (toggle){
		  light_current_pos[2] = light_current_pos[2] - (y - startY) / 10.0;
	  }
	  else{
...
void setLight(){
	...
	glTranslatef(-light_current_pos[0], light_current_pos[1], -light_current_pos[2]);
